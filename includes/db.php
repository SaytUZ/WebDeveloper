...
function db(){
    return new PDO("mysql:host=127.0.0.1;dbname=gr1230chorshan", 'root', '');
}
...
function userInfo(){
    ...
    $driver = $db->prepare($query);
    $driver->execute([isset($_SESSION['id']) ? $_SESSION['id'] : ""]);
...
function addUserPhotos($path){
    ...
    $query = "INSERT INTO `images`(`user_id`, `img_path`, `img_select`) VALUES (?,?,?)";
    $imgDriver = $db->prepare($query);
    return $imgDriver->execute([$userId, $path, 0]);
...
function delPhotos($imgId){
    ...
    if ($result && $result['img_select'] !=1) {
        ...
        $delImg = $db->prepare($query);
        return $delImg->execute([$imgId, $userId]);
    }
    return $result;
}
...
function setInfo($login, $name){
    ...
    $imgDriver = $db->prepare($query);
    return $imgDriver->execute([$login, $name, $userId]);
}